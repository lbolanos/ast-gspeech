...
public class EAgiRequestImpl implements AgiRequest {
    ...
    /**
     * Creates a new EAgiRequestImpl.
     * 
     * @param environment the first lines as received from Asterisk containing
     *            the environment.
     */
    EAgiRequestImpl(final List<String> environment)
    {
        this(buildMap(environment));
    }

    /**
     * Creates a new EAgiRequestImpl based on a preparsed map of parameters.
     *
     * @param request a map representing the AGI request. Keys must not contain the "agi_" or "ogi_" prefix.
     * @since 1.0.0
     */
    private EAgiRequestImpl(final Map<String, String> request)
    {
